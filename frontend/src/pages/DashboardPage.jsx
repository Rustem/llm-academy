...
export default function DashboardPage() {
  ...
  const gmc = (mid) => EX.filter(e => e.m === mid && done[e.id]).length;
  const unlocked = (mod) => dc >= mod.ur;
  const mc = (mod) => T[mod.color];

  const nx = EX.find(e => !done[e.id] && unlocked(MODS.find(m => m.id === e.m)));

  const openExercise = (ex) => navigate(`/exercise/${ex.id}`);
    ...
    <div style={{ background: T.bg, minHeight: "100vh", fontFamily: sn }}>
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "0 24px" }}>
        ...
        <div style={{ display: "flex", gap: 28, marginBottom: 36, flexWrap: "wrap" }}>
          {[
            { l: "Progress", v: `${dc}`, s: "/ 23" },
            { l: "Total XP", v: `${xp}`, c: T.ac },
            { l: "Level", v: lv },
          ].map((s, i) => (
            <div key={i}>
        ...
        {nx && (
          <div style={{
            ...
            <div>
              <span style={{ fontSize: 11, fontWeight: 600, color: T.td, textTransform: "uppercase", letterSpacing: "0.06em" }}>Up next</span>
            ...
            <Btn onClick={() => openExercise(nx)}>Continue →</Btn>
          </div>
        )}
        ...
        {MODS.map((mod, idx) => {
          ...
          const me = EX.filter(e => e.m === mod.id);
          const md = gmc(mod.id);
          return (
            <div key={mod.id}>
              <div style={{ marginBottom: 28, opacity: ul ? 1 : 0.4 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12, flexWrap: "wrap" }}>
                  ...
                  {!ul && <span style={{ fontSize: 11, color: T.td }}>Complete {mod.ur} exercises to unlock</span>}
